...
class HybridHistGBMNumbaV1_0_1:
    """
    ...
    def __init__(self, n_estimators='auto', max_depth='auto', learning_rate='auto',
        ...
        self.interaction_constraints = interaction_constraints          # 'leaf' or 'level'
    ...
    def _pack_tree_list(self, tree_list):
        ...
        features = np.empty(total_nodes, dtype=np.int16)
    ...
    def _get_allowed_features(self, used_features_on_path, all_feature_indices):
        """Given features already used on the path, return allowed features.
        Note: Full Numba-level enforcement is a future improvement.
        """
        if self.interaction_constraints is None:
            return all_feature_indices
        allowed = set()
        if len(used_features_on_path) == 0:
            return all_feature_indices
        for group in self.interaction_constraints:
            group_set = set(group)
            if used_features_on_path.intersection(group_set):
                allowed.update(group_set)
        if len(allowed) == 0:
            return all_feature_indices
        return np.array([f for f in all_feature_indices if f in allowed], dtype=np.int32)
    ...
    def _fit_regression(self, X_bin, y, X_val_bin, y_val, rng):
        ...
        hess = np.zeros(n_samples, dtype=np.float64)
        for m in range(self.n_estimators):
            if getattr(self, 'boosting_type', 'gbdt') == 'dart' and len(self.trees) > 0:
                n_drop = max(1, int(len(self.trees) * self.dart_rate))
                drop_indices = rng.choice(len(self.trees), size=min(n_drop, len(self.trees)), replace=False)
                keep_mask = np.ones(len(self.trees), dtype=bool)
                keep_mask[drop_indices] = False
                F_train = np.full(n_samples, self.init_score_, dtype=np.float64)
                for t_idx in range(len(self.trees)):
                    if keep_mask[t_idx]:
                        tf, tt, tv, tl, tr, nc, sg = self.trees[t_idx]
                        F_train += self.learning_rate * self._predict_tree(X_bin, tf, tt, tv, tl, tr, nc)

            if self.loss == 'mse':
                grad[:] = F_train - y
                hess[:] = 1.0
            elif self.loss == 'mae':
                grad[:] = np.sign(F_train - y)
                hess[:] = 1.0

            if getattr(self, 'custom_objective', None) is not None:
                grad, hess = self.custom_objective(y, F_train)
        ...
        if getattr(self, 'boosting_type', 'gbdt') == 'dart' and len(self.trees) > 0:
            norm_factor = 1.0 / (1.0 - self.dart_rate)
            for t_idx in range(len(self.trees)):
                tf, tt, tv, tl, tr, nc, sg = self.trees[t_idx]
                tv_new = tv * norm_factor
                self.trees[t_idx] = (tf, tt, tv_new, tl, tr, nc, sg)
    ...
    def _fit_binary(self, X_bin, y, X_val_bin, y_val, rng):
        ...
        for m in range(self.n_estimators):
            if getattr(self, 'boosting_type', 'gbdt') == 'dart' and len(self.trees) > 0:
                ...
                keep_mask = np.ones(len(self.trees), dtype=bool)
                keep_mask[drop_indices] = False
                F_train = np.full(n_samples, self.base_score_, dtype=np.float64)
                for t_idx in range(len(self.trees)):
                    if keep_mask[t_idx]:
                        tf, tt, tv, tl, tr, nc, sg = self.trees[t_idx]
                        F_train += self.learning_rate * self._predict_tree(X_bin, tf, tt, tv, tl, tr, nc)
            ...
            if self.objective == 'lambdarank':
                if self.verbose > 0:
                    train_loss = -HybridHistGBMNumbaV1_0_1._compute_ndcg(y, F_train, getattr(self, '_group', None))
                else:
                    train_loss = 0.0
            else:
            ...
            if X_val_bin is not None:
                ...
                F_val += self.learning_rate * val_pred
                if self.objective == 'lambdarank':
                    val_loss = -HybridHistGBMNumbaV1_0_1._compute_ndcg(y_val, F_val, getattr(self, '_group_val', None))
                else:
    ...
    def _fit_multiclass(self, X_bin, y, X_val_bin, y_val, rng):
        ...
        hess = np.zeros(n_samples, dtype=np.float64)
        for m in range(self.n_estimators):
            if getattr(self, 'boosting_type', 'gbdt') == 'dart' and len(self.mc_trees[0]) > 0:
                n_drop = max(1, int(len(self.mc_trees[0]) * self.dart_rate))
                drop_indices = rng.choice(len(self.mc_trees[0]), size=min(n_drop, len(self.mc_trees[0])), replace=False)
                keep_mask = np.ones(len(self.mc_trees[0]), dtype=bool)
                keep_mask[drop_indices] = False
                F_train = np.full((n_samples, self.n_classes_), self.base_score_, dtype=np.float64)
                for k in range(self.n_classes_):
                    for t_idx in range(len(self.mc_trees[k])):
                        if keep_mask[t_idx]:
                            tf, tt, tv, tl, tr, nc, sg = self.mc_trees[k][t_idx]
                            F_train[:, k] += self.learning_rate * self._predict_tree(X_bin, tf, tt, tv, tl, tr, nc)
    ...
    def fit(self, X, y, group=None, X_val=None, y_val=None, group_val=None, eval_set=None):
        ...
        # v1.2.2: Sparse matrix support
        try:
            import scipy.sparse
            if scipy.sparse.issparse(X):
                X = X.toarray()
            if X_val is not None and scipy.sparse.issparse(X_val):
                X_val = X_val.toarray()
        except ImportError:
            pass
        ...
        if getattr(self, 'boosting_type', 'gbdt') == 'dart':
            if not self._is_multiclass and len(self.trees) > 0:
                norm_factor = 1.0 / (1.0 - self.dart_rate)
                for t_idx in range(len(self.trees)):
                    tf, tt, tv, tl, tr, nc, sg = self.trees[t_idx]
                    tv_new = tv * norm_factor
                    self.trees[t_idx] = (tf, tt, tv_new, tl, tr, nc, sg)
            elif self._is_multiclass and self.mc_trees is not None and len(self.mc_trees) > 0 and len(self.mc_trees[0]) > 0:
                norm_factor = 1.0 / (1.0 - self.dart_rate)
                for k in range(self.n_classes_):
                    for t_idx in range(len(self.mc_trees[k])):
                        tf, tt, tv, tl, tr, nc, sg = self.mc_trees[k][t_idx]
                        tv_new = tv * norm_factor
                        self.mc_trees[k][t_idx] = (tf, tt, tv_new, tl, tr, nc, sg)
    ...
    def predict_proba(self, X):
        ...
        if getattr(self, 'n_features_in_', None) is None:
            raise ValueError("This HybridHistGBMNumbaV1_0_1 instance is not fitted yet.")
    ...
    def __repr__(self):
        return (f"HybridHistGBMNumbaV1_0_1(n_estimators={self.n_estimators}, "
    ...
    def production_info(self):
        ...
        print("=" * 60)
        print("  Model:            HybridHistGBMNumbaV1_0_1")
    ...
    def cross_val_score(self, X, y, cv=5, scoring='f1', groups=None, random_state=None):
        ...
        groups : array-like or None
            Group labels for LambdaRank (unused for classification).
        ...
        is_regression = (self.objective == 'regression' or
                         (self.objective == 'auto' and y.dtype.kind == 'f' and len(np.unique(y)) > 20))
        ...
        for train_idx, val_idx in kf.split(X, y):
            ...
            if scoring == 'accuracy':
                ...
                scores.append(f1_score(y_val, model.predict(X_val), average=avg, zero_division=0))
            elif scoring == 'precision':
                avg = 'binary' if len(np.unique(y)) == 2 else 'macro'
                scores.append(precision_score(y_val, model.predict(X_val), average=avg, zero_division=0))
            elif scoring == 'recall':
                avg = 'binary' if len(np.unique(y)) == 2 else 'macro'
                scores.append(recall_score(y_val, model.predict(X_val), average=avg, zero_division=0))
            elif scoring == 'roc_auc':
                proba = model.predict_proba(X_val)
                if proba.shape[1] == 2:
                    scores.append(roc_auc_score(y_val, proba[:, 1]))
                else:
                    scores.append(roc_auc_score(y_val, proba, multi_class='ovr'))
            elif scoring == 'log_loss':
                proba = model.predict_proba(X_val)
                scores.append(log_loss(y_val, proba))
            elif scoring == 'mcc':
                scores.append(matthews_corrcoef(y_val, model.predict(X_val)))
            elif scoring == 'r2':
    ...
    def save_model(self, filepath):
        """Save model to JSON format (safe, portable, version-independent).

        Parameters
        ----------
        filepath : str
            Path to save the model (e.g., 'model.json')
        """
        import json
        model_data = {
            'version': '1.2.2',
            ...
            'n_features_in_': int(self.n_features_in_) if self.n_features_in_ is not None else None,
            'objective': self.objective,
            '_is_multiclass': self._is_multiclass,
            'init_score_': float(self.init_score_),
            'base_score_': float(getattr(self, 'base_score_', 0.0)),
            'n_trees_fitted_': int(self.n_trees_fitted_),
            'best_iteration_': int(self.best_iteration_),
            'train_loss_history_': [float(x) for x in self.train_loss_history_],
            'val_loss_history_': [float(x) for x in self.val_loss_history_],
            '_max_bins': int(self._max_bins),
        }
        # Save serializable params (exclude callables)
        for k, v in self.get_params().items():
            if callable(v):
                continue
            model_data['params'][k] = v

        # Save bin edges
        if self.bin_edges is not None:
            model_data['bin_edges'] = self.bin_edges.tolist()
        if self.feature_bin_counts is not None:
            model_data['feature_bin_counts'] = self.feature_bin_counts.tolist()
        ...
        if not self._is_multiclass:
            ...
            for tree in self.trees:
                model_data['trees'].append({
                    'features': tree[0].tolist(),
                    'thresholds': tree[1].tolist(),
                    'values': tree[2].tolist(),
                    'left': tree[3].tolist(),
                    'right': tree[4].tolist(),
                    'node_count': int(tree[5]),
                    'split_gains': tree[6].tolist() if len(tree) > 6 else [],
                })
        else:
            model_data['mc_trees'] = []
            for k_trees in self.mc_trees:
                class_trees = []
                for tree in k_trees:
                    class_trees.append({
                        'features': tree[0].tolist(),
                ...
                model_data['mc_trees'].append(class_trees)

        # Feature importance
        if self.feature_importances_ is not None:
            model_data['feature_importances_'] = self.feature_importances_.tolist()
        if self.feature_split_counts_ is not None:
            model_data['feature_split_counts_'] = self.feature_split_counts_.tolist()

        # Categorical state
        model_data['_categorical_set'] = list(self._categorical_set)
        if self._is_categorical is not None:
            model_data['_is_categorical'] = self._is_categorical.tolist()
    ...
    def load_model(cls, filepath):
        ...
        params = data['params']
        model = cls(**params)
        model.n_classes_ = data['n_classes_']
        model.classes_ = np.array(data['classes_']) if data['classes_'] is not None else None
        model.n_features_in_ = data['n_features_in_']
        model.objective = data['objective']
        model._is_multiclass = data['_is_multiclass']
        model.init_score_ = data['init_score_']
        model.base_score_ = data.get('base_score_', 0.0)
        model.n_trees_fitted_ = data['n_trees_fitted_']
        model.best_iteration_ = data['best_iteration_']
        model.train_loss_history_ = data['train_loss_history_']
        model.val_loss_history_ = data['val_loss_history_']
        model._max_bins = data['_max_bins']

        # Restore bin edges
        if 'bin_edges' in data:
            model.bin_edges = np.array(data['bin_edges'], dtype=np.float64)
        if 'feature_bin_counts' in data:
            model.feature_bin_counts = np.array(data['feature_bin_counts'], dtype=np.int32)

        # Restore trees
        if 'trees' in data:
            model.trees = []
            for td in data['trees']:
                sg = np.array(td.get('split_gains', [0.0] * td['node_count']), dtype=np.float64)
                tree = (
                    np.array(td['features'], dtype=np.int32),
                    np.array(td['thresholds'], dtype=np.float64),
                    np.array(td['values'], dtype=np.float64),
                    np.array(td['left'], dtype=np.int32),
                    np.array(td['right'], dtype=np.int32),
                    td['node_count'],
                    sg,
                )
        ...
        if 'mc_trees' in data:
            model.mc_trees = []
            for class_trees_data in data['mc_trees']:
                class_trees = []
                for td in class_trees_data:
                    sg = np.array(td.get('split_gains', [0.0] * td['node_count']), dtype=np.float64)
                    tree = (
                        np.array(td['features'], dtype=np.int32),
                        np.array(td['thresholds'], dtype=np.float64),
                ...
                model.mc_trees.append(class_trees)

        # Feature importance
        if 'feature_importances_' in data:
            model.feature_importances_ = np.array(data['feature_importances_'])
        if 'feature_split_counts_' in data:
            model.feature_split_counts_ = np.array(data['feature_split_counts_'])

        # Categorical state
        model._categorical_set = set(data.get('_categorical_set', []))
        if '_is_categorical' in data and data['_is_categorical'] is not None:
            model._is_categorical = np.array(data['_is_categorical'], dtype=np.bool_)

        # Restore runtime attributes that are set during fit
        model.binning_strategy = model.binning_strategy_init if model.binning_strategy_init != 'auto' else 'standard'

        # Restore _nan_bin_id (not stored in JSON — assume no NaN since JSON data is clean)
        if model.n_features_in_ is not None:
            model._nan_bin_id = np.full(model.n_features_in_, -1, dtype=np.int32)

        model._pack_trees()
        return model

class HybridHistGBMRegressorV1_0_1(HybridHistGBMNumbaV1_0_1):
    """
...
class HybridHistGBMRankerV1_0_1(HybridHistGBMNumbaV1_0_1):
    """
    ...
    def predict(self, X):
        ...
        if getattr(self, 'n_features_in_', None) is None:
            raise ValueError("This HybridHistGBMRankerV1_0_1 instance is not fitted yet.")
