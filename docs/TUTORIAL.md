...
model = HybridHistGBMNumbaV2(categorical_features=[0, 3])
model.fit(X_train, y_train)
```
...
model = HybridHistGBMNumbaV2(n_estimators=50)
...
model = HybridHistGBMNumbaV2(monotone_constraints=constraints)
model.fit(X_train, y_train)
```
